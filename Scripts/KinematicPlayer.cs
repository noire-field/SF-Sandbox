...
public class KinematicPlayer : KinematicBody
{
	...
	public override void _Ready()
	{
		...
		ClientSetting.Connect("UserChangeCamera", this, nameof(OnUserChangeCamera));
	...
	public void OnUserChangeCamera(bool toFPS) {
		if(!allowControl) return;
		
		CheckBodyVisibility();
	}
	
	public void CheckBodyVisibility() {
		if(!allowControl) return;
		
		bool visibility = (bool)ClientSetting.Get("firstPersonView") ? false : true;
		
		GetNode("BodyModel").Set("visible", visibility);
		GetNode("Costumes").Set("visible", visibility);
	}
}
